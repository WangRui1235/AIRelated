...
class TabularQLearning(RLAlgorithm):
    ...
    def getAction(self, state: StateT, explore: bool = True) -> ActionT:
        ...
        # BEGIN_YOUR_CODE
        # ε-greedy策略实现
        if explore:#如果是探索模式,use the epsilon-greedy algorithm: with probability |explorationProb|, take a random action.
            if random.random() < explorationProb or not any(self.Q[(state, action)] for action in self.actions):
                return random.choice(self.actions)
            else:
                return max(self.actions, key=lambda action: self.Q[(state, action)])
        else:#explore=False,always choose the maximum Q-value action.
