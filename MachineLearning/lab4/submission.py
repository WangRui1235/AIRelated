...
class ModelBasedMonteCarlo(RLAlgorithm):
    ...
    def getAction(self, state: StateT, explore: bool = True) -> ActionT:
        ...
        if random.random() < explorationProb:
            return random.choice(list(self.actions))  # Random action for exploration
        else:
            if state in self.pi:
                return self.pi[state]  # Use the policy for exploitation
            else:
        ...
        if self.numIters % self.calcValIterEvery == 0:
            ...
            for (state_a, action_a), next_states in self.tCounts.items():
                ...
                for next_state, count in next_states.items():
                    ...
                    expected_reward = self.rTotal[(state_a, action_a)][next_state] / count
